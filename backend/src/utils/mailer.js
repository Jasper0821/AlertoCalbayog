...
async function sendOtpEmail(toEmail, otpCode, expiryMinutes = 7) {
  ...
  const mailOptions = {
    ...
    text: `Your password reset code is: ${otpCode}\n\nThis code expires in ${expiryMinutes} minutes. If you did not request a password reset, please ignore this email.`,
    html: `
      <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:520px;margin:0 auto;background:#f4f7fc;padding:32px 16px;">
        <div style="background:#fff;border-radius:16px;padding:36px 32px;box-shadow:0 4px 24px rgba(0,0,0,0.07);">
          ...
          <p style="font-size:14px;color:#334155;margin:0 0 20px;">
            We received a request to reset your password. Use the code below — it expires in <strong>${expiryMinutes} minutes</strong>.
          </p>
          ...
          <p style="font-size:12px;color:#94a3b8;text-align:center;margin:0;">
            ...
            This code will expire automatically after ${expiryMinutes} minutes.
          </p>
  ...
  try {
    ...
    console.log(`\n✅  [OTP] Password reset code for ${recipient}: ${otpCode}`);
    return info;
  } catch (err) {
...
async function sendResponderApprovalEmail(toEmail, fullName) {
  ...
  if (!transporter) {
    console.log(`[Approval Email Fallback] Account approved for ${recipient} (${responderName})`);
