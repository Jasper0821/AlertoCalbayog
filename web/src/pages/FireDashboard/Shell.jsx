...
function Shell({ activeSection, onNavigate, children }) {
  ...
  return (
    <div className="flex h-screen bg-white font-sans text-slate-900 antialiased overflow-hidden relative">
      ...
      {/* SIDEBAR */}
      <aside className={`fixed inset-y-0 left-0 z-[50] flex w-[300px] flex-col bg-slate-50 border-r border-slate-100 transition-transform duration-300 lg:static lg:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center gap-4 px-8 py-10">
          <div className="grid h-12 w-12 place-items-center rounded-2xl bg-red-600 shadow-lg shadow-red-600/20">
            <BoltIcon className="h-6 w-6 text-white" />
          </div>
          <div>
            <p className="font-display text-xl font-black tracking-[-0.03em] uppercase"><span className="text-red-600 italic">Alerto</span> Calbayog</p>
            <p className="mt-1 text-[10px] font-black uppercase tracking-widest text-slate-500 font-sans">Command Center</p>
          </div>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <item.icon className={`h-5 w-5 ${isActive ? "text-red-600" : "text-slate-400 group-hover:text-red-600 transition-colors"}`} />
        ...
        <div className="px-6 py-8">
          <Link
            to="/"
            className="flex h-14 w-full items-center justify-center gap-3 rounded-2xl bg-white border border-slate-100 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 transition-all hover:bg-red-600 hover:text-white hover:border-red-600 hover:shadow-xl hover:shadow-red-600/20"
          >
      ...
      <main className="flex-1 relative flex flex-col overflow-hidden bg-white">
        <header className="flex h-20 lg:h-24 items-center justify-between border-b border-slate-100 bg-white/80 backdrop-blur-xl px-6 lg:px-12 z-20 shrink-0">
          ...
          <div className="flex items-center gap-4 lg:gap-6">
            <button className="relative hidden sm:grid h-12 w-12 place-items-center rounded-2xl bg-slate-50 border border-slate-100 text-slate-500 hover:bg-slate-100 transition-all">
              ...
              <span className="absolute right-3.5 top-3.5 h-2 w-2 rounded-full bg-red-600 ring-4 ring-white" />
            </button>
            <div className="h-8 w-[1px] bg-slate-100 hidden sm:block" />
            <div className="flex items-center gap-3">
              <div className="text-right hidden xl:block">
                <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Ops Lead</p>
                <p className="mt-0.5 text-[9px] font-bold text-slate-500 uppercase tracking-[0.2em]">Active Shift</p>
              </div>
              <div className="h-10 w-10 lg:h-12 lg:w-12 rounded-2xl bg-slate-900 flex items-center justify-center text-[10px] font-black text-white shadow-xl shadow-slate-900/10">
                AC
              </div>
            </div>
          </div>
        ...
        <section className={`flex-1 p-6 lg:p-12 ${activeSection === "map-report" ? "overflow-hidden" : "overflow-y-auto"} bg-slate-50/30`}>
