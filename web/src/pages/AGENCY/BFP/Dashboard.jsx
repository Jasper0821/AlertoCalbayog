...
// BFP only handles fire emergency reports
function isBfpReport(report) {
  return (report.emergencyType || report.incidentType || report.type || "").toLowerCase() === "fire";
}
...
function BfpDashboard() {
  ...
  const sendDesktopNotification = (report) => {
    ...
    const notif = new Notification(title, {
      body,
      icon: "/logo.png",
      tag: `bfp-alert-${report._id || Date.now()}`,
  ...
  const userName = user.fullName || "BFP Officer";
  const agency = "BFP";
  ...
  // Socket Connection for Real-time Fire Alerts
  useEffect(() => {
    socket.connect();
    const room = "BFP";
    ...
    const onNewEmergencyAlert = (newReport) => {
      ...
      if (reportId && processedAlertsRef.current.has(reportId)) {
        console.log("📡 Duplicate BFP alert ignored:", reportId);
        return;
      }
      if (reportId) {
      ...
      console.log("📡 BFP Command Center received live alert:", newReport);
    ...
    <div className="flex h-screen bg-slate-50 font-sans antialiased overflow-hidden">
      ...
      {activeNav === "live-map" && (
        <button
          ...
          className="fixed top-4 left-4 z-[60] flex items-center justify-center w-10 h-10 rounded-xl bg-[#7f1d1d] text-white shadow-lg hover:bg-[#991b1b] active:scale-95 transition-all"
        >
      ...
      {incomingAlertCount > 0 && (
        <div className="fixed top-4 left-1/2 z-[120] w-[calc(100%-2rem)] max-w-xl -translate-x-1/2">
          <div className="overflow-hidden rounded-2xl border border-red-100 bg-white shadow-2xl shadow-red-950/10">
            <div className="h-1 w-full bg-gradient-to-r from-red-700 via-orange-500 to-yellow-400" />
            <div className="flex items-center justify-between gap-4 px-4 py-3">
              <div className="flex min-w-0 items-center gap-3">
                <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#7f1d1d] text-white shadow-lg shadow-red-900/20">
                ...
                <div className="min-w-0">
                  <p className="truncate text-sm font-black text-slate-900">
                    {incomingAlertCount > 1 ? `${incomingAlertCount} incoming fire reports` : "Incoming fire report"}
                  </p>
                  <p className="truncate text-[11px] font-semibold text-slate-500">
                    {incomingAlertCount > 1
                      ? "Fire reports arrived at the same time."
                      : activeAlert?.emergencyType || "New fire emergency report received."}
                  </p>
                </div>
              </div>
      ...
      {showLogoutModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-[420px] rounded-2xl overflow-hidden shadow-[0_32px_80px_-8px_rgba(0,0,0,0.7)] border border-[#7f1d1d]/60 flex flex-col bg-white animate-zoom-in">
            <div className="h-1 w-full bg-gradient-to-r from-red-700 via-orange-500 to-yellow-400" />

            <div className="bg-[#7f1d1d] px-6 py-4 flex items-center gap-4 shrink-0">
              <div className="w-10 h-10 rounded-xl bg-white/10 border border-white/15 flex items-center justify-center shrink-0">
                <svg className="w-5 h-5 text-orange-300" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
            ...
            <div className="p-6 bg-[#f8fafc]">
              <p className="text-slate-600 text-sm font-medium leading-relaxed">
                Are you sure you want to end your shift session and logout from the BFP command center?
              </p>
            </div>
      ...
      <aside
        ...
        `}
        style={{ background: "#7f1d1d", overflow: "hidden" }}
      >
        {/* Logo / Brand */}
        <div className="flex items-center gap-3 px-4 h-16 shrink-0" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          ...
          <div className={`min-w-0 transition-all duration-300 ${isSidebarOpen || isSidebarHovered ? "opacity-100 w-auto" : "opacity-0 w-0 md:overflow-hidden"}`}>
            ...
            <p className="text-[10px] text-orange-300 font-semibold mt-0.5 tracking-wide whitespace-nowrap">BFP Fire Command</p>
          </div>
        </div>
          ...
          {NAV.map(item => {
            ...
            return (
              ...
              >
                ...
                {/* Icon */}
                <span className={`shrink-0 transition-transform duration-200 ${isActive ? "text-white" : "text-orange-300 group-hover:text-white group-hover:scale-110"}`}>
                  {item.icon}
                </span>
        ...
        <div className="p-2 shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,0.08)" }}>
          <button
            ...
            className="w-full flex items-center gap-3 px-2 py-2.5 rounded-xl text-sm font-medium text-orange-200 hover:bg-red-500/20 hover:text-red-300 transition-all text-left"
        ...
        <header className={`${activeNav === "live-map" ? "hidden" : "flex"} h-16 items-center justify-between bg-white border-b border-slate-200 px-4 md:px-6 shrink-0 gap-4`}>
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="hidden md:flex items-center gap-2 text-sm text-slate-500">
              <span className="font-semibold text-[#7f1d1d]">{pageTitle}</span>
            </div>
            ...
            <div className="relative flex-1 max-w-sm hidden sm:block">
              ...
              <input
                type="search"
                placeholder="Search fire incidents, units, or locations..."
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                className="w-full pl-9 pr-4 py-2 text-xs rounded-lg bg-slate-50 border border-slate-200 focus:bg-white focus:border-[#7f1d1d] focus:ring-2 focus:ring-red-100 text-slate-700 placeholder:text-slate-400 outline-none transition-all"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 shrink-0">
              ...
              {showNotifDropdown && (
                <div className="absolute right-0 top-full mt-2 w-80 bg-white border border-slate-200 rounded-2xl shadow-xl shadow-slate-200/60 z-50 overflow-hidden">
                  ...
                  <div className="px-4 py-2.5 border-t border-slate-100 text-center">
                    <button className="text-xs font-semibold text-red-600 hover:text-red-700" onClick={() => { setActiveNav("queuing"); setShowNotifDropdown(false); stopSiren(); }}>
            ...
            <div className="flex items-center gap-3 pl-2">
              <div className="w-8 h-8 rounded-full overflow-hidden bg-[#7f1d1d] flex items-center justify-center text-white text-xs font-bold ring-2 ring-slate-100 shrink-0">
                <img
                  src={user.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(userName)}&background=7f1d1d&color=fff&bold=true&size=64`}
                  alt="Avatar"
              ...
              <div className="hidden md:block text-left leading-none">
                ...
                <p className="text-[9px] text-slate-400 mt-0.5">{user.rank || "Fire Marshal"}</p>
              </div>
            </div>
      ...
      {activeAlert && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          {/* ── Deep red-smoke backdrop ── */}
          <div
            className="absolute inset-0 bg-[#1a0505]/75 backdrop-blur-sm"
            onClick={closeAlert}
          />
          ...
          {/* ════════════ MODAL CARD ════════════ */}
          <div className="alert-modal-card relative z-10 w-full max-w-[520px] rounded-2xl overflow-hidden shadow-[0_32px_80px_-8px_rgba(0,0,0,0.7)] border border-red-900/60 flex flex-col">

            {/* ── TOP COLOR STRIP ── */}
            <div className="h-1 w-full bg-gradient-to-r from-red-700 via-orange-500 to-yellow-400" />

            {/* ── HEADER ── */}
            <div className="bg-[#7f1d1d] px-6 pt-5 pb-4 flex items-start justify-between shrink-0">
              <div className="flex items-center gap-4">
                <div className="w-11 h-11 rounded-xl bg-white/10 border border-white/15 flex items-center justify-center shrink-0">
                  <svg className="w-6 h-6 text-orange-300" fill="none" stroke="currentColor" strokeWidth="1.6" viewBox="0 0 24 24">
            ...
            <div className="bg-[#6b1a1a] px-6 py-2.5 flex items-center justify-between border-t border-white/5">
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-orange-400 alert-live-dot" />
                <span className="text-orange-300 font-black text-xs uppercase tracking-widest">
                  {activeAlert.emergencyType || "Fire Emergency"}
                </span>
              </div>
              ...
              <div className="px-6 py-5 space-y-0">
                ...
                {[
                  ...
                  >
                    ...
                    <span
                      className={`flex-1 text-sm text-slate-800 ${bold ? "font-bold" : "font-medium"} ${mono ? "font-mono text-[#7f1d1d]" : ""}`}
                    >
                      {value}
                    </span>
                  </div>
                ...
                {Array.isArray(activeAlert.proofPhotos) && activeAlert.proofPhotos.length > 0 && (
                  <div className="mt-3 rounded-xl bg-white border border-red-200 p-3 shadow-xs">
                    ...
                    <div className="grid grid-cols-5 gap-2">
                      {activeAlert.proofPhotos.map((imgUri, idx) => (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="relative aspect-square overflow-hidden rounded-lg border border-slate-200 group hover:ring-2 hover:ring-red-400 transition-all block"
            ...
            <div className="bg-white border-t border-slate-100 px-6 py-4 flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-black text-white bg-[#7f1d1d] hover:bg-[#991b1b] active:scale-95 shadow-lg shadow-red-900/30 transition-all tracking-wide uppercase"
                >
