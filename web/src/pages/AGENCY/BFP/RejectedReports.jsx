...
export default function RejectedReports({ reports = [] }) {
  ...
  const handleDownloadPDF = () => {
    ...
    const htmlContent = `
      <html>
      <head>
        <title>Alerto Calbayog - BFP Rejected Fire Reports</title>
        <style>
          body {
      ...
      <body>
        <div class="header">
          <div class="header-title">
            ...
            <p>Bureau of Fire Protection - Rejected Reports Log</p>
          </div>
          <div class="report-info">
            <p><strong>Generated:</strong> ${new Date().toLocaleString()}</p>
            <p><strong>Total Rejected:</strong> ${filtered.length}</p>
          </div>
        </div>
        ...
        <div class="summary">
          Report summary: Compiled ${filtered.length} rejected records.
        </div>
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col gap-4">
      <div className="shrink-0">
        <h1 className="text-xl font-bold text-slate-800">Rejected Fire Reports</h1>
        <p className="text-sm text-slate-500 mt-0.5">Archive of rejected fire emergency reports.</p>
      </div>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-14 text-center text-sm text-slate-400">
                    No rejected fire records match your filters.
                  </td>
                </tr>
              ) : filtered.map((r, i) => {
