...
import { BellIcon, BoltIcon, LogoutIcon, DashboardIcon, MapIcon, ReportIcon, QueueIcon, ProfileIcon, MenuIcon, SunIcon, MoonIcon } from "./icons.jsx";
      ...
      <aside className={`fixed inset-y-0 left-0 z-[50] flex w-[300px] flex-col bg-slate-50 dark:bg-slate-900/50 dark:backdrop-blur-xl border-r border-slate-100 dark:border-slate-800 transition-all duration-300 lg:static lg:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center gap-4 px-8 py-10">
          ...
          <div>
            ...
            <p className="mt-1 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 font-sans">Command Center</p>
          </div>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <item.icon className={`h-5 w-5 ${isActive ? "text-red-600" : "text-slate-400 dark:text-slate-500 group-hover:text-red-600 transition-colors"}`} />
        ...
        <div className="px-6 py-8">
          <Link
            to="/"
            className="flex h-14 w-full items-center justify-center gap-3 rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 transition-all hover:bg-red-600 hover:text-white hover:border-red-600 hover:shadow-xl hover:shadow-red-600/20"
          >
        ...
        <header className="flex h-20 lg:h-24 items-center justify-between border-b border-slate-100 dark:border-slate-800 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl px-6 lg:px-12 z-20 shrink-0">
          ...
          <div className="flex items-center gap-4 lg:gap-6">
            ...
            <button className="relative hidden sm:grid h-12 w-12 place-items-center rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">
              ...
              <span className="absolute right-3.5 top-3.5 h-2 w-2 rounded-full bg-red-600 ring-4 ring-white dark:ring-slate-950" />
            </button>
            <div className="h-8 w-[1px] bg-slate-100 dark:bg-slate-800 hidden sm:block" />
            <div className="flex items-center gap-3">
              <div className="text-right hidden xl:block">
                <p className="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-widest">Ops Lead</p>
                <p className="mt-0.5 text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em]">Active Shift</p>
              </div>
              <div className="h-10 w-10 lg:h-12 lg:w-12 rounded-2xl bg-slate-900 dark:bg-slate-100 flex items-center justify-center text-[10px] font-black text-white dark:text-slate-900 shadow-xl shadow-slate-900/10">
                AC
              </div>
            </div>
          </div>
