...
export function OverviewSection({ reports = [], isOffline }) {
 ...
 <p className="font-display text-4xl font-black tracking-[-0.07em] text-slate-900 transition-colors">{card.value}</p>
 ...
 <div key={report.id || report._id} className="flex justify-between items-center p-4 bg-white rounded-2xl border border-slate-100">
 <div>
 <p className="text-[10px] font-black uppercase text-red-600">{report.emergencyType}</p>
 <p className="text-sm font-bold text-slate-900">{report.description ||"Active Alert"}</p>
 </div>
