...
function VerifyOTP() {
  ...
  const handleVerify = async (event) => {
    ...
    try {
      const { data } = await api.post("/auth/verify-otp", { email, code: entered });
  ...
  const handleResetPassword = async (event) => {
    ...
    try {
      ...
      setPasswordError(err?.response?.data?.message || "Failed to reset password. Please start over.");
    } finally {
              ...
              <div className="grid gap-1.5 text-left">
                ...
                <div className="relative flex items-center">
                  ...
                  <input
                    ...
                    required
                  />
                  <button type="button" onClick={() => setShowNew(!showNew)} className="absolute right-3.5 text-slate-400 hover:text-slate-600 transition-colors">
                    {showNew ? (
              ...
              <div className="grid gap-1.5 text-left">
                ...
                <div className="relative flex items-center">
                  ...
                  <input
                    ...
                    required
                  />
                  <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-3.5 text-slate-400 hover:text-slate-600 transition-colors">
                    {showConfirm ? (
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l18 18" />
                      </svg>
