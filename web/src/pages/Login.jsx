...
function Login() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Force navigation to dashboard whether registered or not
      navigate("/firedashboard");
    }
  };
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-4 sm:px-6 lg:px-8">
      ...
      <section className="relative w-full max-w-[480px] rounded-[32px] border border-white/10 bg-zinc-950/92 px-7 pb-7 pt-[4.5rem] shadow-[0_30px_90px_rgba(0,0,0,0.45)] backdrop-blur-xl">
        <Link
        ...
        <form className="mt-6" onSubmit={handleSubmit}>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-sm text-stone-300">
            <label className="inline-flex items-center gap-2" htmlFor="remember">
              <input
                className="h-4 w-4 rounded border-stone-500/60 bg-transparent accent-red-500"
                id="remember"
                name="remember"
                type="checkbox"
              />
          ...
          <p className="mt-5 text-center text-sm text-stone-300">
            No account yet?{" "}
            <Link className="font-extrabold text-red-200 transition hover:text-white" to="/register">
              Register
            </Link>
          </p>
        </form>
